...
func searchItems(for query: String, completion: @escaping ([Result]) -> Void) {
    let baseURL = "https://xivapi.com"
    let encodedItemName = query.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!
    let url = URL(string: "\(baseURL)/search?string=\(encodedItemName)&indexes=Recipe&limit=10&fuzzy=true&columns=ID,Icon,Name")!
